...
final class ChuckDateFormatterTests: XCTestCase {
    ...
    let tzNewYork = "America/New_York"
    
    func testBasic() throws {
        let actual = parseChuckMMDDYYYY("03/01/2021", timeZoneID: tzNewYork)
        let expected = df.date(from: "2021-03-01T17:00:00Z")
        XCTAssertEqual(expected, actual)
    }
    
    func testOverrideTimeOfDay() throws {
        let actual = parseChuckMMDDYYYY("03/01/2021", defTimeOfDay: "13:00", timeZoneID: tzNewYork)
        let expected = df.date(from: "2021-03-01T18:00:00Z")
        XCTAssertEqual(expected, actual)
    }

    func testOverrideTimeZone() throws {
        let actual = parseChuckMMDDYYYY("03/01/2021", timeZoneID: "America/Denver")
        let expected = df.date(from: "2021-03-01T19:00:00Z")
        XCTAssertEqual(expected, actual)
    }

    func testOverrideBoth() throws {
        let actual = parseChuckMMDDYYYY("03/01/2021", defTimeOfDay: "13:00", timeZoneID: "America/Denver")
    ...
    func testBankInterestCompound() throws {
        ...
        let actual = parseChuckMMDDYYYY(dateStr, timeZoneID: tzNewYork)
